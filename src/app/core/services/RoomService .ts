...
export class RoomService {
  ...
  // Ajoutez cette méthode qui utilise la pagination Spring Boot
  getRoomsPaged(
    page = 0,
    size = 5,
    sortBy = 'roomNumber',
    ...
    return (
      this.http
        // .get<SpringBootPageResponse<Room>>(`${this.apiUrl}/paged`, { params })
        .get<SpringBootPageResponse<Room>>(`${this.apiUrl}`)
        .pipe(
  ...
  // Modifiez aussi la méthode getRooms pour ne pas utiliser la pagination côté client
  getRooms(filters?: RoomFilters): Observable<Room[]> {
    this.setLoading(true);
    let params = new HttpParams();
    ...
    return this.http
      .get<SpringBootPageResponse<Room>>(this.apiUrl, { params })
      .pipe(
        map((response: SpringBootPageResponse<Room>) => {
          // Retourne seulement le contenu, pas la structure complète
          return response.content;
        }),
        tap((rooms) => {
          this.roomsSubject.next(rooms);
  ...
  //   this.setLoading(true);
  //   return this.http
  //     .get<PagedApiResponse<Room>>(`${this.apiUrl}/paged`, { params })
  //     .pipe(
  ...
  changeRoomStatus(id: number, status: RoomStatus): Observable<Room> {
    return this.http
      .patch<Room>(`${this.apiUrl}/${id}/status`, { status })
      .pipe(
        tap(() => {
          this.clearCache();
          this.refreshRooms();
  ...
  checkAvailability(
    ...
    return this.http
      .get<RoomAvailability>(`${this.apiUrl}/${roomId}/availability`, {
        params,
      })
  ...
  getRoomMaintenance(roomId: number): Observable<MaintenanceRecord[]> {
    return this.http
      .get<MaintenanceRecord[]>(`${this.apiUrl}/${roomId}/maintenance`)
      .pipe(catchError((error) => this.handleError(error)));
  }
  ...
  scheduleMaintenance(
    ...
    return this.http
      .post<MaintenanceRecord>(
        `${this.apiUrl}/${roomId}/maintenance`,
        maintenance
      )
      .pipe(catchError((error) => this.handleError(error)));
  }
  ...
  getAvailableRooms(
    ...
    return this.http
      .get<Room[]>(`${this.apiUrl}/available`, { params })
      .pipe(catchError((error) => this.handleError(error)));
  }
  ...
  bulkUpdateStatus(roomIds: number[], status: RoomStatus): Observable<any> {
    return this.http
      .patch(`${this.apiUrl}/bulk/status`, { roomIds, status })
      .pipe(
        tap(() => {
          this.clearCache();
          this.refreshRooms();
